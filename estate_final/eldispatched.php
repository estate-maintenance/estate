...
$_GET['page']=0;
		...
		if(isset($_POST['print']))
		{
			...
			for($i=0;$i<(count($arr)-1);$i++)
			{
				if(isset($_POST[$arr[$i]]))
				{//changes here	
					$sql="Select username,id,name,designation,location,description,processed,area,time,dispatchedTime,timedesc, contactPerson, contactNumber, room,timing,descText,contact from elcomplaints where id=".$arr[$i]."";
					$result=mysql_query($sql,$conn);
				}
			}
			$row=mysql_fetch_array($result);
...
echo "<p style='margin-top: 50px; margin-left:215px;'><b>SIGNATURE OF ATTENDANT&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;STORES IN - CHARGE&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ASSISTANT ENGINEER</b></p>";
		...
		if(1)
		{//changes here	
			$sql="Select id,name,designation,location,description,processed,area,time,dispatchedTime,contactPerson, contactNumber,timedesc, room,timing,descText,contact from elcomplaints ";
			$status="and";
			$sql.="where ( 1=1 ";
			$status.=" processed=1";
			if ($status=="and")
...
if(isset($_POST['nm']))		
		...
		$sql="Select id,name,designation,location,description,processed,area,time,dispatchedTime, contactPerson, contactNumber,timedesc, room,timing,descText,contact from elcomplaints where processed=0";
...
echo'<a href="eldispatched.php"><center><font color=orange>Dispatched</font></center></a><a href="elunprocessed.php"><center>Unprocessed</center></a>';  
    echo '';
   ?>
   </ul> 
 </div>
...
<tr><td colspan="4" align="center"><input type="submit" name="datesubmit" value="SHOW"/> </form></td>
</tr>
      </table>
...
<div id = "incharge" style="margin-bottom:8px;">
<!--
